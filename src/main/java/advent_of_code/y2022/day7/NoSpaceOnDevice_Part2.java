...
public class NoSpaceOnDevice_Part2 extends AbstractRunnable {

    public static void main(String[] args) {
        new NoSpaceOnDevice_Part2().run();
    }
    ...
    public void run() {
        ...
        int spaceLeft = 70_000_000 - usedSpace;

        int result = directories.stream()
                .map(this::getSize)
                .filter(size -> size + spaceLeft >= 30_000_000)
                .min(Integer::compare)
                .orElse(0);
