...
public class SupplyStacks_Part2 extends AbstractRunnable {

    public static void main(String[] args) {
        new SupplyStacks_Part2().run();
    }
    ...
    public void run() {
        ...
        readLines().forEach(line -> {
            ...
            for (int i = 0; i < nMoves; i++) {
                popped.add(stacks.get(from).pop());
            }

            for (int i = popped.size()-1; i >= 0; i--) {
                stacks.get(to).push(popped.get(i));
            }
        });
        ...
        for (int nStack : stacks.keySet()) {
            result = result.concat(stacks.get(nStack).lastElement());
        }
