...
public class CamelCards_Part1 extends AbstractRunnable {

    public static void main(String[] args) {
        new CamelCards_Part1().run();
    }
    ...
    enum Card {A(12), K(11), Q(10), J(9), T(8), _9(7),
        _8(6), _7(5), _6(4), _5(3), _4(2), _3(1), _2(0);
        final int value;
        Card(int value) {
            this.value = value;
        }
    ...
    public void run() {
        ...
        for (int i = 0; i < orderedDesc.size(); i++) {
            int rank = orderedDesc.size() - i;
            int bid = orderedDesc.get(i).bid;
            result += rank * bid;
        }
    ...
    private HandType getType(Hand hand) {
        ...
        if (groups.size() == 2) {
            Integer firstValue = groups.values().stream().toList().getFirst();
