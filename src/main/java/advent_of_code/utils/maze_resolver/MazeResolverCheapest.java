...
public abstract class MazeResolverCheapest {
    ...
    public MazeResolverCheapest(Matrix maze,
                                char floor,
                                char start,
                                char exit,
                                char wall,
    ...
    private void visit(Position position, List<Position> path, Orientation orientation) {
        ...
        if (solution != null && cost >= solution.cost) {
            return;
        }

        if (maze.check(position.i, position.j, exit)) {
            if (solution == null || cost < solution.cost) {
                solution = new Solution(path, cost);
            }
            return;
        }

        List<Position> neighborsNotVisited = getNeighborsNotVisited(position, path);
        for(Position neighbor : neighborsNotVisited) {
